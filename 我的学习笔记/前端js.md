...
/*name为myform的*/
$("[name = 'myform']").css("background-color","red");
/*交集td中id为td01的*/
...
### ajax
#### 1.为什么要学ajax
Ajax作用：异步通信，局部刷新。不过多请求数据，减少页面、数据库和服务器的压力。
...
Ajax缺点：
- 1.如果一个页面大量使用ajax的话，那么该页面的搜索性能差
- 2.破坏了history历史纪录的机制。

![img_1.png](../img/img_145.png)
#### 2.什么是ajax（synchronous JavaScript And Xml）
使用JSON替换XML。 

![img_3.png](../img/img_147.png)

ajax就是一个其他技术的汇总：
- html +css 完成页面的显示。
- DOM 动态获取标签对象或者显示标签内容
- xml/json 服务区端和客户端之间进行数据转换和传递
- javascript 做总和。 

#### 3.ajax工作流程(会考)
![img_4.png](../img/img_148.png)
...
![img_5.png](../img/img_149.png)
![img_6.png](../img/img_150.png)
![img_11.png](../img/img_155.png)
...
<head>
    ...
    <script type="text/javascript">
        function getlogin(){
            //1.创建ajax引擎对象
            let req = new XMLHttpRequest();
            //2设置请求信息（提交方式，提交地址，提交参数(不写默认为true采用异步请求)）
            let uname = document.getElementsByName("uname")[0].value;
            let pwd = document.getElementsByName("pwd")[0].value;
            let yzpwd = document.getElementsByName("yzpwd")[0].value;

            req.open("GET","http://iwenwiki.com/api/blueberrypai/login.php"+
                    "?user_id="+uname+"&password="+pwd+"&verification_code="+yzpwd);
            // 3.发送请求null表示请求实体（get没有请求实体，post才有）
            req.send(null);
            // 4.获得响应信息（1.相应信息有无异常错误，采用状态码判断2.判断相应信息获取中阶段，采用状态信息）
            req.onreadystatechange=function(){
                ...
                if (req.readyState == 4 && req.status == 200) {
                    ...
                    if (obj.success) {
                        document.getElementsByTagName("span")[0].innerHTML="<b style = 'color : red;'>登录成功</b>";
                    }else{
                        document.getElementsByTagName("span")[0].innerHTML="<b style = 'color : red;'>登录失败</b>";
                    }
                }
            }
        }
        function postlogin(){
            ...
            //2.设置请求信息
            request.open("POST","http://iwenwiki.com/api/blueberrypai/login.php");
            //3.发送请求
            let uname = document.getElementsByName("uname")[1].value;
            let pwd = document.getElementsByName("pwd")[1].value;
            let yzpwd = document.getElementsByName("yzpwd")[1].value;
            //post特有的头信息
            request.setRequestHeader("content-type", "application/x-www-form-urlencoded");
            //post要将请信息放在send中
            request.send("user_id="+uname+"&password="+pwd+"&verification code="+yzpwd);
            //4.获得相应信息
            request.onreadystatechange = function(){
                if (request.readyState == 4 && request.status == 200) {
                    let obj = request.responseText;
                    if (obj.success) {
                        document.getElementsByTagName("span")[1].innerHTML="<b style = 'color : red;'>登录成功</b>";
                    }else{
                        document.getElementsByTagName("span")[1].innerHTML="<b style = 'color : red;'>登录失败</b>";
                    }
                }
            }
        }
        ...
        function getNums(){
            ...
            let req = new XMLHttpRequest;
            //2.设置请求信息
            req.open("GET","http://iwenwiki.com/api/blueberrypai/getIndexInteresting.php");
            //3.发送请求
            req.send(null);
            //4.获得响应信息
            req.onreadystatechange= function(){
                if (req.readyState == 4 && req.status == 200 ) {
                    ...
                    let arrs = jsonobj.interesting;
                    for (let i = 0; i < arrs.length; i++) {
                        let objp = document.createElement("p");
                        objp.innerText = arrs[i].title+"---"+arrs[i].writer;
                        document.getElementById("div1").appendChild(objp);
                    }
                }
            }
        }
            ...
            $("[value = '获得数据']").click(function(){
                ...
                $.ajax({
                    ...
                    success:function(resptxt){
                        console.log(resptxt);
                        let arrs = resptxt.interesting;
                        for (let i = 0; i < arrs.length; i++) {
                            $("<p>"+arrs[i].title+"--"+arrs[i].writer+"</p>").appendTo("#div2");
                        }
                    }
                });
            });
            //一加载就发送异步请求信息
            //使用.get和.post封装ajax异步请求信息
            $.get("http://iwenwiki.com/api/blueberrypai/getIndexInteresting.php",function(resptxt){
                let arrs = resptxt.interesting;
                for (let i = 0; i < arrs.length; i++) {
                    $("<p>"+arrs[i].title+"--"+arrs[i].writer+"</p>").appendTo("#div3");
                }
            });
            $.post("http://iwenwiki.com/api/blueberrypai/getIndexInteresting.php",function(resptxt){
                let arrs = resptxt.interesting;
                for (let i = 0; i < arrs.length; i++) {
                    $("<p>"+arrs[i].title+"--"+arrs[i].writer+"</p>").appendTo("#div4");
                }
            });
        });
    </script>
...
<body>
<div>
    <h1>原生ajax--GET提交获取集合信息</h1>
    <input type="button" value="数据显示" onclick="getNums();">
    <div id="div1">
    </div>
</div>
<div>
    <h1>原生ajax--GET提交获取对象信息</h1>
    用户名：<input type="text" name="uname"><br>
    密码：<input type="password" name="pwd"><br>
    验证码：<input type="text" name="yzpwd"><br>
    <input type="button" value="登录" name="" id="" onclick="getlogin();">
    <span></span>
</div>
<div>
    <h1>原生ajax--POST提交获取对象信息</h1>
    用户名：<input type="text" name="uname"><br>
    密码：<input type="password" name="pwd"><br>
    验证码：<input type="text" name="yzpwd"><br>
    <input type="button" value="登录" name="" id="" onclick="postlogin();">
    <span></span>
</div>
<div>
    <h1>jQuery封装ajax----post提交,获取集合信息</h1>
    <input type="button" value="获得数据">
    <div id="div2"></div>
</div>
<div>
    <h1>jQuery封装ajax----get提交,获取集合信息</h1>
    <div id="div3"></div>
</div>
<div>
    <h1>jQuery封装ajax----postt提交,获取集合信息</h1>
    <div id="div4"></div>
</div>
</body>
</html>
...
- 使用POSt方法采用原生ajax，第一次封装，第二次封装三个方法将响应的集合信息输出到
一个表格中。
...
<head>
    ...
    <script type="text/javascript" src="../../js/jquery-1.8.3.js"></script>
    <script type="text/javascript">
        //原生ajax异步请求信息并将响应信息集合输出到一个表格中
        function getInfo1(){
            let request = new XMLHttpRequest;
            request.open("POST","http://iwenwiki.com/api/blueberrypai/getIndexInteresting.php");
            request.setRequestHeader("content-type", "application/x-www-form-urlencoded");
            request.send(null);
            request.onreadystatechange = function(){
                if (request.readyState == 4 && request.status == 200) {
                    let obj = request.responseText;
                    let jsonobj = JSON.parse(obj);
                    let arrays = jsonobj.interesting ;
                    for (let i = 0; i < arrays.length; i++) {
                        document.getElementsByName("tt")[i].innerText = arrays[i].title;
                        document.getElementsByName("w")[i].innerText = arrays[i].writer;
                        document.getElementsByName("t")[i].innerText = arrays[i].time;
                        document.getElementsByName("c")[i].innerText = arrays[i].content;
                        document.getElementsByName("i")[i].innerText = arrays[i].img;
                    }
                }
            }
        }
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            //第一阶段使用jQuery封装ajax异步请求信息并将响应信息集合输出到一个表格中
            $.ajax({
                "type":"POST",
                "url":"http://iwenwiki.com/api/blueberrypai/getIndexInteresting.php",
                "content-type":"application/x-www-form-urlencoded",
                success:function(requestTxt){
                    let arrays = requestTxt.interesting;
                    for (let i = 0; i < arrays.length; i++) {
                        //jQuery挑选索引位于i的标签对象
                        // $("td[name = tt1]:eq("+i+")").text(arrays[i].title);
                        $("td[name = tt1]").eq(i).text(arrays[i].title);
                        $("td[name = w1]").eq(i).text(arrays[i].writer);
                        $("td[name = t1]").eq(i).text(arrays[i].time);
                        $("td[name = c1]").eq(i).text(arrays[i].content);
                        $("td[name = i1]").eq(i).text(arrays[i].img);
                    }
                }
            });
        });
        //第二阶段使用jQuery封装ajax异步请求信息并将响应信息集合输出到一个表格中
        //第二阶段简化就直接在tr标签中循环创建td标签，节省body空间
        $.post("http://iwenwiki.com/api/blueberrypai/getIndexInteresting.php",function(requestTxt){
            let arrays = requestTxt.interesting;
            for (let i = 0; i < arrays.length; i++) {
                $("<td >"+arrays[i].title+"</td>").appendTo($("tr[name = tr"+i+"]"));
                $("<td >"+arrays[i].writer+"</td>").appendTo($("tr[name = tr"+i+"]"));
                $("<td >"+arrays[i].time+"</td>").appendTo($("tr[name = tr"+i+"]"));
                $("<td >"+arrays[i].content+"</td>").appendTo($("tr[name = tr"+i+"]"));
                $("<td >"+arrays[i].img+"</td>").appendTo($("tr[name = tr"+i+"]"));
            }
        });
    </script>

</head>
<body onload="getInfo1();">
<table>
    <th colspan="5" class="th1">原生ajax--POST提交获取集合信息</th>
    <tr>
        <td>title</td>
        <td>writer</td>
        <td>time</td>
...
<table>
    <tr>
        <th colspan="5" class="th1" >jQuery第一阶段封装ajax--POST提交获取集合信息</th>
    </tr>
    <tr>
        <td>title</td>
        <td>writer</td>
...
<br>
<br>
<!-- 采用方法不同第一阶段jQuery使用直接往td中.text（）改变文本，第二阶段jQuery采用$("<td></td>")创建标签加入tr标签中 -->
<table>
    <tr>
        <th colspan="5" class="th1">jQuery第二阶段封装ajax--POST提交获取集合信息</th>
    </tr>
    <tr>
        <td>title</td>
        <td>writer</td>
